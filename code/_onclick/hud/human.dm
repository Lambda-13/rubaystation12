...
/obj/screen/food/Click(var/location, var/control, var/params)
	if(istype(usr) && usr.nutrition_icon == src)
		switch(icon_state)
			if("nutrition0")
				to_chat(usr, SPAN_WARNING("Наелся."))
			if("nutrition1")
				to_chat(usr, SPAN_NOTICE("Не хочу есть."))
			if("nutrition2")
				to_chat(usr, SPAN_NOTICE("Сделал бы небольшой перекус."))
			if("nutrition3")
				to_chat(usr, SPAN_WARNING("Хочу кушать."))
			if("nutrition4")
				to_chat(usr, SPAN_DANGER("Меня мучает голод!"))

/obj/screen/drink/Click(var/location, var/control, var/params)
	if(istype(usr) && usr.hydration_icon == src)
		switch(icon_state)
			if("hydration0")
				to_chat(usr, SPAN_WARNING("Перепил."))
			if("hydration1")
				to_chat(usr, SPAN_NOTICE("Не хочу пить."))
			if("hydration2")
				to_chat(usr, SPAN_NOTICE("Немного хочу выпить."))
			if("hydration3")
				to_chat(usr, SPAN_WARNING("Хочу пить."))
			if("hydration4")
				to_chat(usr, SPAN_DANGER("Меня мучает жажда!"))

/obj/screen/bodytemp/Click(var/location, var/control, var/params)
	if(istype(usr) && usr.bodytemp == src)
		switch(icon_state)
			if("temp4")
				to_chat(usr, SPAN_DANGER("Плавлюсь!"))
			if("temp3")
				to_chat(usr, SPAN_DANGER("Горю!"))
			if("temp2")
				to_chat(usr, SPAN_DANGER("У меня перегрев."))
			if("temp1")
				to_chat(usr, SPAN_WARNING("Вокруг жарко."))
			if("temp-4")
				to_chat(usr, SPAN_DANGER("Леденею!"))
			if("temp-3")
				to_chat(usr, SPAN_DANGER("Замерзаю!"))
			if("temp-2")
				to_chat(usr, SPAN_WARNING("Вокруг холодно"))
			if("temp-1")
				to_chat(usr, SPAN_NOTICE("Вокруг прохладно."))
			else
				to_chat(usr, SPAN_NOTICE("Моя температура тела в допустимой норме."))
...
/obj/screen/pressure/Click(var/location, var/control, var/params)
	if(istype(usr) && usr.pressure == src)
		switch(icon_state)
			...
			if("pressure1")
				to_chat(usr, SPAN_WARNING("Давление неестественно высокое."))
			if("pressure-1")
				to_chat(usr, SPAN_WARNING("Давление неестественно низкое."))
			if("pressure-2")
				to_chat(usr, SPAN_DANGER("Ваккум!"))
			else
				to_chat(usr, SPAN_NOTICE("Давление вокруг меня достаточное."))
...
/obj/screen/toxins/Click(var/location, var/control, var/params)
	if(istype(usr) && usr.toxin == src)
		if(icon_state == "tox0")
			...
			to_chat(usr, SPAN_DANGER("Воздух токсичен для меня!"))

/obj/screen/oxygen/Click(var/location, var/control, var/params)
	if(istype(usr) && usr.oxygen == src)
		if(icon_state == "oxy0")
			to_chat(usr, SPAN_NOTICE("Воздух поступает в мои лёгкие."))
		else
			to_chat(usr, SPAN_DANGER("Не могу дышать!"))
