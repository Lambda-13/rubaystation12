/obj/item/device/chameleon
	name = "хамелеонский проектор"
	icon = 'lambda/icons/obj/chameleon_projector.dmi'
	icon_state = "shield0"
...
/obj/item/device/chameleon/afterattack(atom/target, mob/user , proximity)
	...
	if(!active_dummy)
		if(istype(target,/obj/item) && !istype(target, /obj/item/disk/nuclear))
			...
			to_chat(user, "<span class='notice'>Сканирую [target].</span>")
...
/obj/item/device/chameleon/proc/toggle()
	...
	if(active_dummy)
		...
		qdel(active_dummy)
		active_dummy = null
		to_chat(usr, "<span class='notice'>Отключаю [src].</span>")
		...
		qdel(O)
		to_chat(usr, "<span class='notice'>Включаю [src].</span>")
		var/obj/effect/overlay/T = new/obj/effect/overlay(get_turf(src))
		T.icon = 'icons/effects/effects.dmi'
		flick("emppulse",T)
		QDEL_IN(T, 8)
...
/obj/effect/dummy/chameleon/attackby()
	for(var/mob/M in src)
		to_chat(M, "<span class='warning'>Проектор отключается.</span>")
	master.disrupt()
