/client/verb/ooc(message as text)
	set name = "OOC"
	set category = "OOC"

	sanitize_and_communicate(/decl/communication_channel/ooc, src, message)

/client/verb/looc(message as text)
	set name = "LOOC"
	set desc = "Local OOC, seen only by those in view. Remember: Just because you see someone that doesn't mean they see you."
	set category = "OOC"

	sanitize_and_communicate(/decl/communication_channel/ooc/looc, src, message)

/client/verb/fix_chat()
	set name = "Fix Chat"
	set category = "OOC"
	if (!chatOutput || !istype(chatOutput))
		var/action = alert(src, "Chat Output прислал непонятные данные от тебя!\nПересоздадим их?", "Че?", "Пересоздать выходные данные", "Отмена")
		if (action != "Пересоздать выходные данные")
			return
		chatOutput = new /chatOutput (src)
		chatOutput.start()
		action = alert(src, "Гунчат перезагружён, всё работает?", "", "Всё работает", "Не-а")
		if (action == "Всё работает")
			log_game("GOONCHAT: [key_name(src)] Had to fix their goonchat by re-creating the chatOutput datum")
		else
			chatOutput.load()
			action = alert(src, "А теперь? (Подожди немного (оно может около минуты загружаться))", "", "Да", "Нет")
			if (action == "Да")
				log_game("GOONCHAT: [key_name(src)] Had to fix their goonchat by re-creating the chatOutput datum and forcing a load()")
			else
				action = alert(src, "Чтож, у меня кончились идеи. Можешь попытаться перезайти в дримсикер (закрыть открыть игру) или переподключиться.\nМогу включить старый догуновский чат", "", "Спасибо не надо", "Включи старый чат")
				if (action == "Включи старый чат")
					winset(src, "output", "is-visible=true;is-disabled=false")
					winset(src, "browseroutput", "is-visible=false")
				log_game("GOONCHAT: [key_name(src)] Failed to fix their goonchat window after recreating the chatOutput and forcing a load()")

	else if (chatOutput.loaded)
		var/action = alert(src, "ChatOutput похоже уже загружен\nВы хотите, чтобы я принудительно перезагрузился, стер журнал чата или просто обновил окно чата, потому что оно сломалось/пропало?", "Фиксик", "Загрузи всё заного", "Обнови окно", "Отмена")
		switch (action)
			if ("Загрузи всё заного")
				chatOutput.loaded = FALSE
				chatOutput.start() //this is likely to fail since it asks , but we should try it anyways so we know.
				action = alert(src, "Гунчат перезагружён, всё работает?", "", "Всё работает", "Не-а")
				if (action == "Всё работает")
					log_game("GOONCHAT: [key_name(src)] Had to fix their goonchat by forcing a start()")
				else
					chatOutput.load()
					action = alert(src, "А теперь? (Подожди немного (оно может около минуты загружаться))", "", "Да", "Нет")
					if (action == "Да")
						log_game("GOONCHAT: [key_name(src)] Had to fix their goonchat by forcing a load()")
					else
						action = alert(src, "Чтож, у меня кончились идеи. Можешь попытаться перезайти в дримсикер (закрыть открыть игру) или переподключиться.\nМогу включить старый догуновский чат", "", "Спасибо не надо", "Включи старый чат")
						if (action == "Включи старый чат")
							winset(src, "output", "is-visible=true;is-disabled=false")
							winset(src, "browseroutput", "is-visible=false")
						log_game("GOONCHAT: [key_name(src)] Failed to fix their goonchat window forcing a start() and forcing a load()")

			if ("Обнови окно")
				chatOutput.showChat()
				action = alert(src, "Окошко с чатом было перезагружено, всё работает?", "", "Всё работает", "Не-а, загрузи всё заного")
				if (action == "Всё работает")
					log_game("GOONCHAT: [key_name(src)] Had to fix their goonchat by forcing a show()")
				else
					chatOutput.loaded = FALSE
					chatOutput.load()
					action = alert(src, "А теперь? (подожди немного)", "", "Да", "Нет")
					if (action == "Да")
						log_game("GOONCHAT: [key_name(src)] Had to fix their goonchat by forcing a load()")
					else
						action = alert(src, "Чтож, у меня кончились идеи. Можешь попытаться перезайти в дримсикер (закрыть открыть игру) или переподключиться.\nМогу включить старый догуновский чат", "", "Спасибо не надо", "Включи старый чат")
						if (action == "Включи старый чат")
							winset(src, "output", "is-visible=true;is-disabled=false")
							winset(src, "browseroutput", "is-visible=false")
						log_game("GOONCHAT: [key_name(src)] Failed to fix their goonchat window forcing a show() and forcing a load()")
		return

	else
		chatOutput.start()
		var/action = alert(src, "Мануально чат загружен или загружается, честно не ебу просто сообщи всё работает или нет.", "", "Всё работает", "Не-а")
		if (action == "Всё работает")
			log_game("GOONCHAT: [key_name(src)] Had to fix their goonchat by manually calling start()")
		else
			chatOutput.load()
			alert(src, "А теперь? (Подожди немного (оно может около минуты загружаться))", "", "Да", "Нет")
			if (action == "Да")
				log_game("GOONCHAT: [key_name(src)] Had to fix their goonchat by manually calling start() and forcing a load()")
			else
				action = alert(src, "Чтож, у меня кончились идеи. Можешь попытаться перезайти в дримсикер (закрыть открыть игру) или переподключиться.\nМогу включить старый догуновский чат", "", "Спасибо не надо", "Включи старый чат")
				if (action == "Включи старый чат")
					winset(src, "output", list2params(list("on-show" = "", "is-disabled" = "false", "is-visible" = "true")))
					winset(src, "browseroutput", "is-disabled=true;is-visible=false")
				log_game("GOONCHAT: [key_name(src)] Failed to fix their goonchat window after manually calling start() and forcing a load()")
