...
/datum/category_item/player_setup_item/physical/equipment/content()
	...
	for(var/datum/category_group/underwear/UWC in GLOB.underwear.categories)
		...
		. += "<br>"
	. += "Сумка: <a href='?src=\ref[src];change_backpack=1'><b>[pref.backpack.name]</b></a>"
	...
	. += "<br>"
	. += "Сенсоры на одежде: <a href='?src=\ref[src];change_sensor_setting=1'>[pref.sensor_setting]</a><br />"
	. += "Блокировка сенсоров: <a href='?src=\ref[src];toggle_sensors_locked=1'>[pref.sensors_locked ? "Заблок." : "Разблок."]</a><br />"
...
/datum/category_item/player_setup_item/physical/equipment/OnTopic(var/href,var/list/href_list, var/mob/user)
	if(href_list["change_underwear"])
		...
		var/new_backpack = input(user, "Выберите рюкзак:", CHARACTER_PREFERENCE_INPUT_TITLE, pref.backpack) as null|anything in backpacks_by_name
		...
		var/switchMode = input("Выберите режим сенсоров:", "Режим сенсоров", pref.sensor_setting) as null | anything in SUIT_SENSOR_MODES
