...
/datum/category_item/player_setup_item/antagonism/basic/content(var/mob/user)
	. +="<b>Антагонист:</b><br>"
	. +="Место появления аплинка: <a href='?src=\ref[src];add_source=1'>Добавить</a><br>"
	for(var/entry in pref.uplink_sources)
		var/decl/uplink_source/US = entry
		. +="[US.name] <a href='?src=\ref[src];move_source_up=\ref[US]'>⬆</a> <a href='?src=\ref[src];move_source_down=\ref[US]'>⬇</a> <a href='?src=\ref[src];remove_source=\ref[US]'>❌</a><br>"
		if(US.desc)
			. += "<font size=1>[US.desc]</font><br>"
	if(!pref.uplink_sources.len)
		. += "<span class='warning'>Вы не получите аплинк, если не добавите место появления!</span>"
	. +="<br>"
	. +="Эксплуатируемая информация:<br>"
...
/datum/category_item/player_setup_item/antagonism/basic/OnTopic(var/href,var/list/href_list, var/mob/user)
	if(href_list["add_source"])
		var/source_selection = input(user, "Выберите место аплинка", CHARACTER_PREFERENCE_INPUT_TITLE) as null|anything in (list_values(uplink_sources_by_name) - pref.uplink_sources)
	...
	if(href_list["exploitable_record"])
		var/exploitmsg = sanitize(input(user,"Установите здесь информацию о себе, которую можно использовать.","Эксплуатируемая информация", html_decode(pref.exploit_record)) as message|null, MAX_PAPER_MESSAGE_LEN, extra = 0)
