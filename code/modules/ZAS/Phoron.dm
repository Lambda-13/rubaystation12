...
/mob/living/carbon/human/pl_effects()
	...
	if(vsc.plc.SKIN_BURNS)
		if(!pl_head_protected() || !pl_suit_protected())
			burn_skin(0.75)
			if(prob(20)) to_chat(src, "<span class='danger'>Моя кожа горит!</span>")
			updatehealth()
	...
	if(vsc.plc.GENETIC_CORRUPTION)
		if(rand(1,10000) < vsc.plc.GENETIC_CORRUPTION)
			randmutb(src)
			to_chat(src, "<span class='danger'>Высокий уровень токсинов заставляет меня мутировать!</span>")
			domutcheck(src,null)
...
/mob/living/carbon/human/proc/burn_eyes()
	...
	if(E && !E.phoron_guard)
		if(prob(20)) to_chat(src, "<span class='danger'>Глаза горят!</span>")
		E.damage += 2.5
		eye_blurry = min(eye_blurry+1.5,50)
		if (prob(max(0,E.damage - 15) + 1) &&!eye_blind)
			to_chat(src, "<span class='danger'>Я ослеп!</span>")
			eye_blind += 20
