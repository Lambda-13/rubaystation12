...
/obj/effect/overmap/visitable/sector/exoplanet/proc/get_base_image()
	var/image/base = image('lambda/icons/skybox/planet.dmi', "base")
	base.color = get_surface_color()
	return base

/obj/effect/overmap/visitable/sector/exoplanet/proc/generate_planet_image()
	skybox_image = image('lambda/icons/skybox/planet.dmi', "")
	...
	if (water_color) //TODO: move water levels out of randommap into exoplanet
		var/image/water = image('lambda/icons/skybox/planet.dmi', "water")
	...
	if (atmosphere && atmosphere.return_pressure() > SOUND_MINIMUM_PRESSURE)
		...
		var/image/clouds = image('lambda/icons/skybox/planet.dmi', "weak_clouds")

		if (water_color)
			clouds.overlays += image('lambda/icons/skybox/planet.dmi', "clouds")

		clouds.color = atmo_color
		skybox_image.overlays += clouds

		var/image/atmo = image('lambda/icons/skybox/planet.dmi', "atmoring")
		skybox_image.underlays += atmo

	var/image/shadow = image('lambda/icons/skybox/planet.dmi', "shadow")
	shadow.blend_mode = BLEND_MULTIPLY
	skybox_image.overlays += shadow

	var/image/light = image('lambda/icons/skybox/planet.dmi', "lightrim")
	skybox_image.overlays += light

	if (prob(20))
		var/image/rings = image('lambda/icons/skybox/planet_rings.dmi')
